...
class CDCK2(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        # real_output = x[:, :, t_samples*50:(t_samples+self.timestep)*50-1]
        # loss = mse(result, real_output)
        # nce += loss
    ...
    def predict(self, x, hidden):
        ...
        pred = pred.transpose(0, 1)
        # result = output[:, -1, :].view(batch, 256).detach().cpu().numpy()
        # np.save('/data1/ryan/contexts/context_training_103_' + str(number), result)
        # print(result.shape)

        return pred  # return every frame

    def showcurve(self, x, hidden):
        timestep = 12
        batch = x.size()[0]
        ratio1 = 0.3
        ratio2 = 0.5

        z1 = self.encoder(x[:, :, :int(x.size()[2] * ratio1)])
        z1 = z1.transpose(1, 2)
        output1, hidden = self.gru(z1, hidden)

        z2 = self.encoder(x[:, :, :int(x.size()[2] * ratio2)])
        z2 = z2.transpose(1, 2)
        output2, hidden = self.gru(z2, hidden)

        result1 = output1[:, -1, :].view(batch, 256).detach().cpu().numpy()
        result2 = output2[:, -1, :].view(batch, 256).detach().cpu().numpy()
        pred1 = torch.empty((timestep, batch, 512)).float()  # e.g. size 12*1*512
        pred2 = torch.empty((timestep, batch, 512)).float()  # e.g. size 12*1*512
        for i in np.arange(0, timestep):
            linear = self.Wk[i]
            pred[i] = linear(result)  # Wk*c_t e.g. size 64*512
