...
from src.data_reader.dataset import RawDataset, ReverseRawDataset, RawXXreverseDataset, TrainMagnetDataset, ValMagnetDataset
...
def main():
    ...
    training_set = TrainMagnetDataset(args.train_raw, args.train_list, args.audio_window)
    ...
    validation_set = ValMagnetDataset(args.validation_raw, args.validation_list, args.audio_window)
    ...
    validation_loader = data.DataLoader(validation_set, batch_size=args.batch_size, shuffle=True, **params) # set shuffle to True
    # nanxin optimizer
    ...
    """
    # Output the final context vector
    final_loader = data.DataLoader(training_set, batch_size=100, shuffle=True, **params)
    i = 0
    for batch_idx, ex in enumerate(final_loader):
        ex = ex.float().unsqueeze(1).to(device)  # add channel dimension
